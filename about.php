
                                 
                                 
       <hr class="featurette-divider" id="aboutUs">

       <div class="featurette">

         <img class="featurette-image pull-right" src="img/team2.png">
        <h2 class="featurette-heading">About Us. <span class="muted">We're a pretty good team.</span></h2>
        <p class="lead">Donec ullamcorper nulla non metus auctor fringilla. Vestibulum id ligula porta felis euismod semper. Praesent commodo cursus magna, vel scelerisque nisl consectetur. Fusce dapibus, tellus ac cursus commodo.</p>

        <div class="row">
<button class="btn btn-primary pull-right" type="button">Contact us</button>
        </div>

    <div class="row-fluid top-buffer-large">

      <img class="featurette-image team-photo pull-left" src="img/team-will.png">
      <div class="span7 well white-font">
          <img class="well-callout callout-left" src="img/well-callout.png">
          
          <div class="span11 pull-left">  
            <h2 class="pull-left full-width">Will Kerswell - Developer</h2>
              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed varius lobortis sem in dignissim. 
              Cras aliquam porttitor scelerisque. Nullam blandit malesuada varius. Curabitur imperdiet massa 
              eu odio tempor sed molestie elit imperdiet. Nam rhoncus leo nec ligula laoreet vel aliquet felis</p>
              <p>
              scelerisque. Nunc nec leo lectus, a ultricies tellus. Donec at arcu enim, in posuere ante. Etiam 
              vel tortor ac enim pulvinar volutpat. Morbi ut est ut neque volutpat pellentesque. Morbi rhoncus 
              imperdiet consequat. In ac velit sed justo vestibulum tincidunt. Proin id lacus id libero bibendum 
              commodo.</p>

              <button class="btn btn-primary top-buffer-xsmall" type="button">Contact us</button>

          </div><!--END SPAN-->
          
      </div> 

    </div>


    <div class="row-fluid top-buffer-large">
      <img class="featurette-image team-photo pull-right" src="img/team-perry.png">

      <div class="span7 well white-font pull-right">
          <img class="well-callout callout-right rotate-180" src="img/well-callout.png">
          
          <div class="span11 white-font">  
            <h2 class="full-width pull-right">Perry Steward - Designer</h2>
              <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed varius lobortis sem in dignissim. 
              Cras aliquam porttitor scelerisque. Nullam blandit malesuada varius. Curabitur imperdiet massa 
              eu odio tempor sed molestie elit imperdiet. Nam rhoncus leo nec ligula laoreet vel aliquet felis</p>
              <p>
              scelerisque. Nunc nec leo lectus, a ultricies tellus. Donec at arcu enim, in posuere ante. Etiam 
              vel tortor ac enim pulvinar volutpat. Morbi ut est ut neque volutpat pellentesque. Morbi rhoncus 
              imperdiet consequat. In ac velit sed justo vestibulum tincidunt. Proin id lacus id libero bibendum 
              commodo.</p>

              <button class="btn btn-primary top-buffer-xsmall" type="button">Contact us</button>
          </div><!--END SPAN-->
      </div> 
    </div>

  </div><!--close featurette-->
